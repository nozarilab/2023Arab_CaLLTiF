...
for r in range(0,num_rep_macaque):
    ...
    dataframe  = pd.DataFrame(np.array(macaque_all_data_full[r]), columns = nodes_macaque_full)
    ...
    for l in range(0, len(mvgc_lags)):
        for a in range(0, len(mvgc_alpha)):
                ...
                # np.savez(os.path.join(results_folder,'MVGC_macaque_full_sweep_both_parameters_all_reps'),mvgc_link_matrix_all_macaque = mvgc_link_matrix_all_macaque, mvgc_macaque_execution_time = mvgc_macaque_execution_time)
